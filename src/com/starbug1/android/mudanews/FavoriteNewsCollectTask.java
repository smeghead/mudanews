...
public class FavoriteNewsCollectTask extends AsyncTask<String, Integer, List<FavoriteMonth>> {
	...
	protected List<FavoriteMonth> doInBackground(String... params) {
		List<FavoriteMonth> result = new ArrayList<FavoriteMonth>();
		...
		try {
			...
			for (Date begin : months) {
				...
				c = db.rawQuery(
						...
						"group by f.id " +
	 					"order by published_at desc ", new String[]{
	...
	protected void onPostExecute(List<FavoriteMonth> result) {
		...
		ListView list = (ListView)activity_.findViewById(R.id.favorite_blocks);
		
		FavoriteMonthListAdapter adapter = new FavoriteMonthListAdapter(activity_);
